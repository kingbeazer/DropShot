    ...
    CompetitionSchedulerService scheduler) : ControllerBase
{
    ...
    {
        await using var db = dbFactory.CreateDbContext();
        var hostClubId = await db.Competition
            .Where(c => c.CompetitionID == id)
            .Select(c => new { c.CompetitionID, c.HostClubId })
            .FirstOrDefaultAsync();
        if (hostClubId is null) return NotFound();
        if (!await authzService.CanEditCompetitionAsync(User, hostClubId.HostClubId)) return Forbid();

        await scheduler.ScheduleAsync(id, new ScheduleFixturesRequest(ScheduleDeleteMode.AllIncomplete));
        return Ok();
    }
    // ── League Table ──────────────────────────────────────────────────────────
